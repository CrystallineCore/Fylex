...
# Ensure FYLEX_HOME exists on first use, not at import time
def _ensure_fylex_home():
    FylexConfig.FYLEX_HOME.mkdir(parents=True, exist_ok=True)
...
def copy_with_conflict_resolution(mode: str, src: Path | str, dest: Path | str, backup: Path | str, dry_run: bool, algo: str, resolve: str, 
    ...
    if isinstance(action, Path):
        ...
        if (verify):
            if (get_or_update_file_hash(src, algo) != get_or_update_file_hash(dest_file, algo)):
                ...
                if mode == "move":
                    ...
                    logging.info(f"[PROGRESS: {progress()}%] File copied and hash verified using {algo} successfully.")
                with thread_lock:
                    state.dupe_candidates.setdefault(dest_file.stat().st_size, []).append(get_or_update_file_hash(dest_file, algo, chunk_size))
        ...
        if mode == "move":
            ...
            target_path = Path(backup) / src.name
    ...
    for entry in reversed(entries):
        ...
        try:
            if op == "copy":
                ...
                if dest and dest.exists():
                    if not dry_run:
                        fast_move(dest, src, algo, dry_run)
...
def redo(p_id: int, verbose: bool = True, force: bool = False, summary: Path | str = None, dry_run: bool = False) -> int:
    ...
    if p_id_dry_run:
        logging.warning("[REDO] Skipping dry-run entry (cannot rollback a dry-run).")
        return -1
